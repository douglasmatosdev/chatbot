...
const { dialogYesNo, dialogStartConversation, welcomeDialogMenuHelper, menuFirstOptions, requestDemo, msgConfirmRequestDemo, menuNavigation } = require('./dialogs.js');
			...
			if (val.ftrigger) {
				if (val.ftrigger==="welcome") {
					...
					if (val.userNameStartDialog) {
						...
						await turnContext.sendActivity(`Olá ${userName} !!!`);
						await turnContext.sendActivity(TYPING);
						console.log(menuFirstOptions(userName));
						await turnContext.sendActivity(
							{
								attachments:[menuFirstOptions(userName)]
		...
		if (val.ftrigger) {
			...
			if (val.id=="whoami") {
				...
				// await turnContext.sendActivity(COGNITIVE_MOCKED_DATA.loadMockData().whoami8);
					await turnContext.sendActivity({
					attachments:[menuNavigation()]
				})
				
			} else if(val.id=="submitRequest") {
				
				await turnContext.sendActivity(TYPING);
				await turnContext.sendActivity(demoRequestSubmit());
			...
			} else if(val.id == "submitRequestDemo") {
				await turnContext.sendActivity(TYPING);
				await turnContext.sendActivity({
					attachments:[requestDemo()]
					
				});

			} else if (val.id=="confirmMsgRequestDemo") {
					userEmail=val.userEmailRequestDemo;
					await turnContext.sendActivity({
						attachments: [
							msgConfirmRequestDemo(userName, userEmail)
						]
					});
					...
					// await turnContext.sendActivity({
					// 	attachments:[
					// 		menuFirstOptions(userName)
					// 	]
					// })

			} else if(val.id=="menuReturn") {
				await turnContext.sendActivity(TYPING);
				await turnContext.sendActivity({
					attachments:[
						menuFirstOptions()
			...
			} else if(val.id=="moreOptions") {
				await turnContext.sendActivity(TYPING);
				await turnContext.sendActivity({
					attachments:[
						welcomeDialogMenuHelper()
					]
				})


			} else if(val.id == "learnMoreIntelieLive") {
				await turnContext.sendActivity(TYPING);
				await turnContext.sendActivity({
					attachments:[COGNITIVE_MOCKED_DATA.learnMoreIntelieLive()]
				})
				// await turnContext.sendActivity({
				// 	attachments:[menuNavigation()]
				// })
